...
static void (*handler[SDL_NUMEVENTS])(SDL_Event *) = {
    [SDL_KEYDOWN] = kpress,          [SDL_VIDEORESIZE] = resize,  [SDL_VIDEOEXPOSE] = expose,
    [SDL_ACTIVEEVENT] = activeEvent, [SDL_MOUSEMOTION] = bmotion, [SDL_MOUSEBUTTONDOWN] = bpress,
...
// upscale 320x240x16 -> 1280x720x32 without rotation
void upscale4x3(uint16_t *restrict src, uint32_t *restrict dst) {
    uint32_t x, y, pix, dpix;
    for (y = 0; y < 240; y++) {
        for (x = 0; x < 320; x++) {
            pix = *src++;

            // Convert the 16-bit pixel to 32-bit
            ...
            // Write the 4x3 block of the converted pixel to the destination
            uint32_t *destPtr = dst + (y * 3 * 1280) + (x * 4);
            destPtr[0]        = dpix;
            destPtr[1]        = dpix;
            destPtr[2]        = dpix;
            destPtr[3]        = dpix;

            destPtr += 1280;  // Move to the next row in the block
            destPtr[0] = dpix;
            destPtr[1] = dpix;
            destPtr[2] = dpix;
            destPtr[3] = dpix;
...
int utf8decode(char *s, long *u) {
    ...
    if ((n == 1 && *u < 0x80) || (n == 2 && *u < 0x800) || (n == 3 && *u < 0x10000) ||
        (*u >= 0xD800 && *u <= 0xDFFF)) {
        goto invalid;
    }
...
void ttynew(void) {
    ...
    switch (pid = fork()) {
        ...
        default:
            ...
            if (opt_io) {
                iofd = (!strcmp(opt_io, "-")) ? STDOUT_FILENO
...
void tsetchar(char *c, Glyph *attr, int x, int y) {
    ...
    if (attr->mode & ATTR_GFX) {
        if (c[0] >= 0x41 && c[0] <= 0x7e && vt100_0[c[0] - 0x41]) { c = vt100_0[c[0] - 0x41]; }
    }
...
void tsetattr(int *attr, int l) {
    ...
    for (i = 0; i < l; i++) {
        switch (attr[i]) {
            case 0:
                term.c.attr.mode &=
                    ~(ATTR_REVERSE | ATTR_UNDERLINE | ATTR_BOLD | ATTR_ITALIC | ATTR_BLINK);
                term.c.attr.fg = defaultfg;
                term.c.attr.bg = defaultbg;
                break;
            case 1: term.c.attr.mode |= ATTR_BOLD; break;
            case 3: /* enter standout (highlight) */ term.c.attr.mode |= ATTR_ITALIC; break;
            case 4: term.c.attr.mode |= ATTR_UNDERLINE; break;
            case 5: term.c.attr.mode |= ATTR_BLINK; break;
            case 7: term.c.attr.mode |= ATTR_REVERSE; break;
            case 21:
            case 22: term.c.attr.mode &= ~ATTR_BOLD; break;
            case 23: /* leave standout (highlight) mode */ term.c.attr.mode &= ~ATTR_ITALIC; break;
...
void tsetmode(bool priv, bool set, int *args, int narg) {
    ...
    for (lim = args + narg; args < lim; ++args) {
        if (priv) {
            switch (*args) {
                break;
                case 1: /* DECCKM -- Cursor key */ MODBIT(term.mode, set, MODE_APPKEYPAD); break;
                case 5: /* DECSCNM -- Reverse video */
                    mode = term.mode;
                    MODBIT(term.mode, set, MODE_REVERSE);
                    if (mode != term.mode) redraw();
                    break;
                case 6: /* XXX: DECOM -- Origin */ break;
                case 7: /* DECAWM -- Auto wrap */ MODBIT(term.mode, set, MODE_WRAP); break;
                case 8: /* XXX: DECARM -- Auto repeat */ break;
                case 0: /* Error (IGNORED) */
                case 12: /* att610 -- Start blinking cursor (IGNORED) */ break;
                case 25: MODBIT(term.c.state, !set, CURSOR_HIDE); break;
                case 1000: /* 1000,1002: enable xterm mouse report */
                    MODBIT(term.mode, set, MODE_MOUSEBTN);
                    break;
                case 1002: MODBIT(term.mode, set, MODE_MOUSEMOTION); break;
            ...
            switch (*args) {
                case 0: /* Error (IGNORED) */ break;
                case 2: /* KAM -- keyboard action */ MODBIT(term.mode, set, MODE_KBDLOCK); break;
                case 4: /* IRM -- Insertion-replacement */
                    MODBIT(term.mode, set, MODE_INSERT);
                    break;
                case 12: /* XXX: SRM -- Send/Receive */ break;
                case 20: /* LNM -- Linefeed/new line */ MODBIT(term.mode, set, MODE_CRLF); break;
                default: fprintf(stderr, "erresc: unknown set/reset mode %d\n", *args); break;
            }
        }
    }
}
...
void csihandle(void) {
    switch (csiescseq.mode) {
        ...
        case 'c': /* DA -- Device Attributes */
            if (csiescseq.arg[0] == 0) ttywrite(VT102ID, sizeof(VT102ID) - 1);
            break;
        ...
        case 'g': /* TBC -- Tabulation clear */
            switch (csiescseq.arg[0]) {
                case 0: /* clear current tab stop */ term.tabs[term.c.x] = 0; break;
        ...
        case 'J': /* ED -- Clear screen */
            ...
            switch (csiescseq.arg[0]) {
                ...
                case 1: /* above */
                    ...
                    break;
                case 2: /* all */ tclearregion(0, 0, term.col - 1, term.row - 1); break;
                default: goto unknown;
            }
            break;
        case 'K': /* EL -- Clear line */
            switch (csiescseq.arg[0]) {
                case 0: /* right */
                    tclearregion(term.c.x, term.c.y, term.col - 1, term.c.y);
                    break;
                case 1: /* left */ tclearregion(0, term.c.y, term.c.x, term.c.y); break;
                case 2: /* all */ tclearregion(0, term.c.y, term.col - 1, term.c.y); break;
            }
            break;
        ...
        case 'Z': /* CBT -- Cursor Backward Tabulation <n> tab stops */
            DEFAULT(csiescseq.arg[0], 1);
            while (csiescseq.arg[0]--) tputtab(0);
            break;
        ...
        case 'r': /* DECSTBM -- Set Scrolling Region */
            ...
            break;
        case 's': /* DECSC -- Save cursor position (ANSI.SYS) */ tcursor(CURSOR_SAVE); break;
        ...
        case ']': /* OSC -- Operating System Command */
            switch (p[0]) {
                ...
                case '2':
                     ...
                     */
                    if (p[1] == ';') { SDL_WM_SetCaption(strescseq.buf + 2, NULL); }
                    break;
                case ';': SDL_WM_SetCaption(strescseq.buf + 1, NULL); break;
                case '4': /* TODO: Set color (arg0) to "rgb:%hexr/$hexg/$hexb" (arg1) */ break;
                default:
            ...
            break;
        case 'k': /* old title set compatibility */ SDL_WM_SetCaption(strescseq.buf, NULL); break;
...
void tputc(char *c, int len) {
    ...
    if (term.esc & ESC_STR) {
        switch (ascii) {
            case '\033': term.esc = ESC_START | ESC_STR_END; break;
    ...
    if (control) {
        switch (ascii) {
            case '\t': /* HT */ tputtab(1); return;
            case '\b': /* BS */ tmoveto(term.c.x - 1, term.c.y); return;
            case '\r': /* CR */ tmoveto(0, term.c.y); return;
            case '\f': /* LF */
            case '\v': /* VT */
            case '\n': /* LF */
            ...
            case '\033': /* ESC */
                ...
                term.esc = ESC_START;
                return;
            case '\016': /* SO */ term.c.attr.mode |= ATTR_GFX; return;
            case '\017': /* SI */ term.c.attr.mode &= ~ATTR_GFX; return;
            case '\032': /* SUB */
            case '\030': /* CAN */ csireset(); return;
        ...
        if (term.esc & ESC_CSI) {
            ...
            switch (ascii) {
                case '0': /* Line drawing set */ term.c.attr.mode |= ATTR_GFX; break;
                case 'B': /* USASCII */ term.c.attr.mode &= ~ATTR_GFX; break;
                case 'A': /* UK (IGNORED) */
                case '<': /* multinational charset (IGNORED) */
                case '5': /* Finnish (IGNORED) */
                case 'C': /* Finnish (IGNORED) */
                case 'K': /* German (IGNORED) */ break;
                default: fprintf(stderr, "esc unhandled charset: ESC ( %c\n", ascii);
            }
            term.esc = 0;
            ...
            switch (ascii) {
                ...
                case 'k': /* old title set compatibility */
                    ...
                    term.esc |= ESC_STR;
                    break;
                case '(': /* set primary charset G0 */ term.esc |= ESC_ALTCHARSET; break;
                case ')': /* set secondary charset G1 (IGNORED) */
                case '*': /* set tertiary charset G2 (IGNORED) */
                case '+': /* set quaternary charset G3 (IGNORED) */ term.esc = 0; break;
                case 'D': /* IND -- Linefeed */
    ...
    if (term.c.x + 1 < term.col) tmoveto(term.c.x + 1, term.c.y);
...
void sdltermclear(int col1, int row1, int col2, int row2) {
    if (xw.win == NULL) return;
    SDL_Rect  r = {borderpx + col1 * xw.cw, borderpx + row1 * xw.ch, (col2 - col1 + 1) * xw.cw,
                   (row2 - row1 + 1) * xw.ch};
...
void sdlinit(void) {
    ...
    if (!(screen = SDL_SetVideoMode(1280, 720, 32, SDL_HWSURFACE | SDL_DOUBLEBUF))) {
        fprintf(stderr, "Unable to set video mode: %s\n", SDL_GetError());
        exit(EXIT_FAILURE);
    }
    xw.win  = SDL_CreateRGBSurface(SDL_SWSURFACE, xw.w, xw.h, 16, 0xF800, 0x7E0, 0x1F,
                                   0);  // console screen
    screen2 = SDL_CreateRGBSurface(SDL_SWSURFACE, xw.w, xw.h, 16, 0xF800, 0x7E0, 0x1F,
                                   0);  // for keyboardMix
...
void sdlresettitle(void) { SDL_WM_SetCaption(opt_title ? opt_title : "st", NULL); }
...
void drawregion(int x1, int y1, int x2, int y2) {
    ...
    for (y = y1; y < y2; y++) {
        ...
        for (x = x1; x < x2; x++) {
            ...
            if (ib > 0 &&
                (!(new.state &GLYPH_SET) || ATTRCMP(base, new) || ib >= DRAW_BUF_SIZ - UTF_SIZ)) {
...
void activeEvent(SDL_Event *ev) {
    switch (ev->active.type) {
        ...
        case SDL_APPINPUTFOCUS: focus(ev); break;
    }
}
...
void kpress(SDL_Event *ev) {
    ...
    for (i = 0; i < LEN(shortcuts); i++) {
        if ((ksym == shortcuts[i].keysym) &&
            ((CLEANMASK(shortcuts[i].mod) & CLEANMASK(e->keysym.mod)) ==
             CLEANMASK(e->keysym.mod)) &&
            shortcuts[i].func) {
...
void resize(SDL_Event *e) {
    if (e->resize.w == xw.w && e->resize.h == xw.h) return;
    ...
    for (i = 1; i < argc; i++) {
        switch (argv[i][0] != '-' || argv[i][2] ? -1 : argv[i][1]) {
            ...
            case 't':
                ...
                break;
            case 'q': active = show_help = 0; break;
            case 'v':
            default: die(USAGE);
        }
    }

    if (atexit(sdlshutdown)) { fprintf(stderr, "Unable to register SDL_Quit atexit\n"); }
    /*
    char path[PATH_MAX];
